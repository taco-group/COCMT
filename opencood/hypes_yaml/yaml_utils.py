...
### add load point cloud params for CMT-L
def load_point_cloud_params(param):

    cav_lidar_range = param['preprocess']['cav_lidar_range']
    if 'voxel_size' in param['preprocess']['args']:
        voxel_size = param['preprocess']['args']['voxel_size']
    else:
        voxel_size = param['preprocess']['args']['lidar_preprocess']['args']['voxel_size']
...
def load_mmcv_param(param):
    """
    Use mmcv to load BEVformer's parameters so that it is compatible with original implementation
    ...
    for mmcv_file_path_ptr in param['mmcv_file_path_ptr']:
        mmcv_file_path_ptr = mmcv_file_path_ptr.split(".")
        mmcv_file_path = param
        for attr_name in mmcv_file_path_ptr:
            mmcv_file_path = mmcv_file_path[attr_name]
        mmcv_file_path = os.path.join(fileDirname, mmcv_file_path)
        cfg = Config.fromfile(mmcv_file_path)
        update_content = param
        for attr_name in mmcv_file_path_ptr[:-1]:
            update_content = update_content[attr_name]
        update_content['cfg'] = cfg
    return param
...
def save_yaml(data, save_name):
    ...
    if "mmcv_file_path_ptr" in data:
        fileDirname = "/".join(save_name.split("/")[:-1])
        for mmcv_file_path_ptr in param['mmcv_file_path_ptr']:
            mmcv_file_path_ptr = mmcv_file_path_ptr.split(".")
            ...
            for attr_name in mmcv_file_path_ptr:
                mmcv_file_path = mmcv_file_path[attr_name]
            mmcv_file_name = mmcv_file_path.split("/")[-1]
            assert ".py" in mmcv_file_name, "mmcv file must be .py file"
            mmcv_file_path = os.path.join(fileDirname, mmcv_file_name)

            update_content = param
            for attr_name in mmcv_file_path_ptr[:-1]:
                update_content = update_content[attr_name]
            cfg = update_content['cfg']
            # cfg.dump(mmcv_file_path)
            update_content['cfg'] = ""# will reload and parse it when loading yaml
