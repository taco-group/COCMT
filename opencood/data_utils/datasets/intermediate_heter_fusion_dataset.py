...
from opencood.utils.transformation_utils import dist_two_pose
...
def getIntermediateheterFusionDataset(cls):
    ...
    class IntermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.anchor_free_flag = params['postprocess'].get('anchor_free', False)
            if self.anchor_free_flag is not True:
            ...
            # 获取 multi_range_deepsupervision_flag 的值，如果不存在则默认为 False
            self.mrds_flag = params['postprocess']['anchor_args'].get('multi_range_deepsupervision', False)
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if sensor_type == "camera":
                ...
                for idx, img in enumerate(camera_data_list):
                    ...
                    lidar2camera = self.get_lidar2camera(params, idx)
                    ...
                    lidar2cameras.append(lidar2camera)
                    ...
                    f"image_inputs_{modality_name}":
                        {
                            ...
                            "lidar2cameras": torch.stack(lidar2cameras),
                        }
                    }
                )
            ...
            if self.mrds_flag:
                s_ego_object_bbx_center, s_ego_object_bbx_mask, s_ego_object_ids = \
                    self.generate_object_center([selected_cav_base],
                                                               ego_pose_clean, multi_range_label_flag=True)
            ...
            if self.mrds_flag:
                selected_cav_processed.update(
                    {'s_ego_object_bbx_center': s_ego_object_bbx_center[s_ego_object_bbx_mask == 1],
                     's_ego_object_ids': s_ego_object_ids})
        ...
        def __getitem__(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                distance = dist_two_pose(selected_cav_base['params']['lidar_pose'], ego_lidar_pose)
            ...
            if len(lidar_pose_list[0]) == 4:
                lidar_poses = np.array(lidar_pose_list).reshape(-1, 4, 4)  # [N_cav, 4, 4]
                lidar_poses_clean = np.array(lidar_pose_clean_list).reshape(-1, 4, 4)  # [N_cav, 4, 4]
            else:
                lidar_poses = np.array(lidar_pose_list).reshape(-1, 6)  # [N_cav, 6]
                lidar_poses_clean = np.array(lidar_pose_clean_list).reshape(-1, 6)  # [N_cav, 6]

            # (max_cav, 4, 4)
            transformation_matrix_full = []
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                transformation_matrix_full.append(
                    selected_cav_processed['transformation_matrix'])

                object_stack.append(selected_cav_processed['object_bbx_center'])
                object_id_stack += selected_cav_processed['object_ids']
                if self.mrds_flag:
                    s_ego_object_stack.append(selected_cav_processed['s_ego_object_bbx_center'])
                    s_ego_object_id_stack += selected_cav_processed['s_ego_object_ids']
            ...
            # padding
            transformation_matrix_full = np.stack(transformation_matrix_full)
            padding_eye = np.tile(np.eye(4)[None], (self.max_cav - len(
                transformation_matrix_full), 1, 1))
            transformation_matrix_full = np.concatenate(
                [transformation_matrix_full, padding_eye], axis=0)
            ...
            if self.params['fusion']['dataset'] == 'dairv2x':
                ...
                if self.mrds_flag:
                    s_ego_unique_indices = \
                        [s_ego_object_id_stack.index(x) for x in set(s_ego_object_id_stack)]
                    s_ego_object_stack = np.vstack(s_ego_object_stack)
                    s_ego_object_stack = s_ego_object_stack[s_ego_unique_indices]
            ...
            if self.mrds_flag:
                s_ego_object_bbx_center = \
                    np.zeros((self.params['postprocess']['max_num'], 7))
                s_ego_mask = np.zeros(self.params['postprocess']['max_num'])
                s_ego_object_bbx_center[:s_ego_object_stack.shape[0], :] = s_ego_object_stack
                s_ego_mask[:s_ego_object_stack.shape[0]] = 1
            ...
            if self.mrds_flag:
                s_ego_label_dict = \
                    self.post_processor.generate_label(
                        gt_box_center=s_ego_object_bbx_center,
                        anchors=self.anchor_box,
                        mask=s_ego_mask,
                        multi_range_label_flag=True)
                label_dict.update(s_ego_label_dict)
            ...
            processed_data_dict['ego'].update({
                'transformation_matrix_full': transformation_matrix_full})
        ...
        def collate_batch_train(self, batch):
            ...
            transformation_matrix_full_all_batch = []
            ...
            for i in range(len(batch)):
                ...
                transformation_matrix_full_all_batch.append(
                    ego_dict['transformation_matrix_full'])
            ...
            # (B,max_cav,4,4)
            transformation_matrix_full_all_batch = \
                torch.from_numpy(np.stack(transformation_matrix_full_all_batch)).float()
            ...
            output_dict['ego'].update({
                'transformation_matrix_full': transformation_matrix_full_all_batch})
