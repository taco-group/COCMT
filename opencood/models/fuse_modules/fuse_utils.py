...
def regroup_query_or_reference_points(object_query, record_len, max_len):
    """
    Regroup object_query based on the record_len.

    Parameters
    ----------
    object_query : torch.Tensor
        Num CAVs, Num Queries, Embbed Dim -> N Q C
    ...
    """
    cum_sum_len = list(np.cumsum(torch_tensor_to_numpy(record_len)))
    split_features = torch.tensor_split(object_query,
                                        cum_sum_len[:-1])
    regroup_features = []
    mask = []
