...
def inference_late_fusion(batch_data, model, dataset, qbm=False):
    """
    ...
    if qbm is not True:
        ...
        for cav_id, cav_content in batch_data.items():
            output_dict[cav_id] = model(cav_content, test_flag=True)
...
def inference_early_fusion(batch_data, model, dataset, qbm=False):
    """
    ...
    if qbm is not True:
        ...
        output_dict['ego'] = model(cav_content, test_flag=True)
...
def inference_intermediate_fusion(batch_data, model, dataset, qbm=False):
    """
    Model inference for early fusion.
    ...
    gt_box_tensor : torch.Tensor
        The tensor of gt bounding box.
    """
    return_dict = inference_early_fusion(batch_data, model, dataset, qbm=qbm)
    return return_dict
