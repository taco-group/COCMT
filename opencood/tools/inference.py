...
def test_parser():
    ...
    parser.add_argument('--epoch', required=False, type=int,
                        default=-1, help='which epoch to load for inference')
    parser.add_argument('--lightning', action='store_true',
                        help='whether trained by pytorch lightning')
    parser.add_argument('--qbm', action='store_true',
                        help='query based detection method')
...
def main():
    ...
    if opt.lightning:
        from lightning_train import MyModel
        lightning_model = MyModel(hypes)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        lightning_model.to(device)
        _, lightning_model = cmt_utils.load_saved_model(opt.model_dir, lightning_model,
                                                epoch=opt.epoch)
        model = lightning_model.model
        model.eval()
        opt.note += f"_epoch{opt.epoch}"
    else:
